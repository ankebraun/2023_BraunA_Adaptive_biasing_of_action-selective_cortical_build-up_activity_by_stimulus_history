...
def get_source_space(subject):
    ...
    computation.
    """
    subjects_without_mri = [2, 4, 11, 21]
    if subject in subjects_without_mri:
        return mne.setup_source_space(subject, spacing='oct7',
                                      subjects_dir=subjects_dir,
                                      add_dist=False)
    else:                                     
...
def get_leadfield(subject, raw_filename, epochs_filename, trans_filename,
                  conductivity=(0.3, 0.006, 0.3), njobs=1, bem_sub_path='bem'):
...
def replace_fiducials(info, fiducials):
    ...
    fids = _make_dig_points(**fiducials)
    info = info.copy()
    dig = info['dig']
