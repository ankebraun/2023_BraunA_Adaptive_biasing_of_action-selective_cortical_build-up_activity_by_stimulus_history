"""
From Niklas Wilming et al. Nature Communications 2020
...
def complex_tfr(x, time, est_val=None, est_key=None, sf=400., foi=None,
    ...
    print ('foi', foi)
    print ('cycles', cycles)
    print ('time_bandwidth', time_bandwidth)
    pdb.set_trace()    
    y = _compute_tfr(
...
def setup_filters(info, forward, data_cov, noise_cov, labels,
                  reg=0.05, pick_ori='max-power', njobs=1):
...
def reconstruct_broadband(
        ...
        accumulate_func=accumulate, njobs=1):
...
def get_filter(info, forward, data_cov, noise_cov, label=None, reg=0.05,
    ...
    print("forward['src']")               
    print(forward['src'])
    print("forward['src'].kind")     
    print(forward['src'].kind)    
    print("data_cov", data_cov)
