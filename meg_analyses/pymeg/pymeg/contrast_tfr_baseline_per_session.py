...
def baseline_per_sensor_apply(tfr, baseline):
    ...
    def div(x):
        ...
        print(baseline.index.isin([areas], level='area'))
        print(areas)
        bval = float(baseline.loc[baseline.index.isin([freqs], level='freq')& baseline.index.isin([areas], level='area')])
...
def load_tfr_contrast(data_globstring, base_globstring, meta_data, conditions,
    ...
    # tfr_data = tfr_data.rename(index={'lh.JWDG.lr_aIPS-lh-lh': 'lh.JWDG.lr_aIPS-lh', 'rh.JWDG.lr_aIPS-rh-rh': 'rh.JWDG.lr_aIPS-rh'})
    # tfr_data = tfr_data.rename(index={'lh.JWDG.lr_IPS_PCeS-lh-lh': 'lh.JWDG.lr_IPS_PCeS-lh', 'rh.JWDG.lr_IPS_PCeS-rh-rh': 'rh.JWDG.lr_IPS_PCeS-rh'})
